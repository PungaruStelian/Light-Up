...
testtimelimit(30). % in seconds % use more for vmchecker
...
tests([], _, Ex, 0) :- !, format("[~w] Fără teste.~n", [Ex]).
...
test(Ex, ech(Text, [Cond|Conds])) :- !,
    ...
    swritef(Msg0, '%w nu produce nicio solutie pentru ca interogarea eșuează.', [Text]),
    testCall(Ex, findall(_, P, A), Msg0),
    (   A = [], !, failure(Ex, Msg0) ; true ),
    forall((P), (
               swritef(Msg, '%w pentru soluția %w a predicatului %w',
                       [Cond, Vars, Text]),
               testCall(Ex, Q, Msg))),
